...
to the `spatcore/` submodule of this repo; *(WFS-DIY)* marks files in the
sibling app repo (github.com/pob31/WFS-DIY, locally `D:\dev\WFS_DIY_v1`).

**Prerequisite**: XOA currently pins spatcore `7d293e4`, which predates all of
the below. Step zero is bumping the submodule to spatcore `c8fd7bf` or later —
the binaural module landed in `c11cbd3`, the head-tracker plugin ABI +
calibration groundwork in `b67d630`, and the macOS PlatformDynLib fix in
`65310d7`. Follow the usual bump ritual (WFS-DIY pins the same branch; keep
both apps on compatible pins).
