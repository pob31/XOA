...
/* WFS-DIY vendoring note: Z_PREFIX is forced on so this copy (used by the
 * vendored libmysofa) never collides with the zlib JUCE embeds in juce_core
 * (which exports the unprefixed C symbols). Everything that includes THIS
 * zconf.h — our zlib TUs and libmysofa's calls — renames consistently. */
#ifndef Z_PREFIX
#  define Z_PREFIX 1
#endif
/* z_errmsg already starts with z_ so the Z_PREFIX table skips it — rename it
 * explicitly, it collides with the copy JUCE embeds too. */
#define z_errmsg wfs_z_errmsg
